...
func EstablishConnection() (*websocket.Conn, error) {
	...
	if err != nil {
		global.Logger.Error(fmt.Sprintf("failed to get connection %s", err.Error()))
		return nil, err
	}
...
func SubScribeAndListen(topics []string) error {
	...
	if err != nil {
		global.Logger.Error("failed to get connection ", zap.Error(err))
		return err
	}
